...
// (NotifyFixedFunctionPicaRegisterChanged and individual Sync* methods removed
// in favour of SyncDrawState() called once per Draw() — backport Azahar #1059)
...
void RasterizerOpenGL::SyncAndUploadLUTsLF() {
    ...
    if (is_gles && !GLAD_GL_OES_texture_buffer) { //gvx64
        ...
        while (pica.lighting.lut_dirty) {
            ...
            glActiveTexture(TextureUnits::TextureBufferLUT_RG.Enum());
            glBindTexture(GL_TEXTURE_2D, texture_buffer_lut_rg.handle);
            const int x_offset = (index % 4) * 256;
            const int y_offset = index / 4;
...
void RasterizerOpenGL::SyncAndUploadLUTs() {
    ...
    if (is_gles && !GLAD_GL_OES_texture_buffer) { //gvx64
        ...
        if (pica.proctex.noise_lut_dirty) {
            ...
            glBindTexture(GL_TEXTURE_2D, texture_buffer_lut_rg.handle);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 128, 1, GL_RG, GL_FLOAT, new_data.data());
            fs_data.proctex_noise_lut_offset = 0;
            fs_data_dirty = true;
        }
        ...
        if (pica.proctex.color_map_dirty) {
            ...
            fs_data.proctex_color_map_offset = 128;
            fs_data_dirty = true;
        }
        ...
        if (pica.proctex.alpha_map_dirty) {
            ...
            fs_data.proctex_alpha_map_offset = 256;
            fs_data_dirty = true;
        }
        ...
        if (pica.proctex.diff_lut_dirty) {
            ...
            fs_data.proctex_diff_lut_offset = 256;
            fs_data_dirty = true;
        }
    } else {
