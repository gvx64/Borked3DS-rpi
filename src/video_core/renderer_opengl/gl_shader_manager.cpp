...
//gvx64    std::size_t shaders_to_load = limited_loading ? MAX_STARTUP_SHADERS : raws.size();
...
// gvx64 - begin - Multi-threaded precompiled shader loading for separable shaders
    ...
    if (impl->separable) {
        ...
        bool limited_loading = raws.size() > MAX_STARTUP_SHADERS;
...
//gvx64        }
        ...
        if (!strict_context_required && shaders_to_load > 100) {
            ...
            // Multi-threaded path for separable shaders
            const std::size_t num_workers{std::max(1U, std::thread::hardware_concurrency())};
            const std::size_t bucket_size{shaders_to_load / num_workers};
...
//gvx64                     shaders_to_load, raws.size(), num_workers);
            ...
            for (std::size_t i = 0; i < num_workers; ++i) {
                ...
                const std::size_t end{is_last_worker ? shaders_to_load : start + bucket_size};

                contexts[i] = emu_window.CreateSharedContext();
                contexts[i]->DoneCurrent();

                threads[i] = std::thread([&, start, end, context = contexts[i].get()]() {
                    const auto scope = context->Acquire();
                    LoadPrecompiledShader(start, end, raws, decompiled, dumps);
                });
            }
            ...
            LoadPrecompiledShader(0, shaders_to_load, raws, decompiled, dumps);
        }
    } else {
...
//gvx64        LOG_CRITICAL(Render_OpenGL, "DEBUG: Taking NON-SEPARABLE path");
        ...
        const std::size_t MAX_STARTUP_SHADERS = Settings::values.startup_shader_load_limit.GetValue();
        bool limited_loading = dumps.size() > MAX_STARTUP_SHADERS;
        std::size_t programs_to_load = limited_loading ? MAX_STARTUP_SHADERS : dumps.size();
...
//gvx64            }
            ...
            // Convert dumps map to vector for easier indexing
            std::vector<std::pair<u64, ShaderDiskCacheDump>> dumps_vec(dumps.begin(), dumps.end());

            const std::size_t bucket_size{programs_to_load / num_workers};
...
//gvx64                     programs_to_load, dumps_vec.size(), num_workers);
            ...
            for (std::size_t i = 0; i < num_workers; ++i) {
                const bool is_last_worker = i + 1 == num_workers;
                const std::size_t start{bucket_size * i};
                const std::size_t end{is_last_worker ? programs_to_load : start + bucket_size};
                ...
                threads[i] = std::thread([&, start, end, context = contexts[i].get()]() {
                    ...
                    for (std::size_t idx = start; idx < end; ++idx) {
                        ...
                        const auto& [unique_identifier, dump] = dumps_vec[idx];
                        const auto decomp = decompiled.find(unique_identifier);

                        if (decomp == decompiled.end() || decomp->second.sanitize_mul != accurate_mul) {
                            continue;
                        }

                        OGLProgram shader = GeneratePrecompiledProgram(dump, supported_formats, impl->separable);
                        if (shader.handle != 0) {
                            std::scoped_lock lock(mutex);
                            impl->program_cache.emplace(unique_identifier, std::move(shader));
                        } else {
