...
/*std::string GenerateTrivialVertexShader(bool use_clip_planes, bool separable_shader) {
    ...
    // Always declare version on first line
    out += "#version 310 es\n";

    // Conditionally enable useful extensions
    out += R"(
...
#extension GL_EXT_clip_cull_distance : enable
    ...
    // Input attributes
    out += fmt::format(R"(
layout(location = {}) in vec4 vert_position;
...
layout(location = {}) in vec3 vert_view;
    ...
    // Output declarations from helper
    out += GetVertexInterfaceDeclaration(true, use_clip_planes, separable_shader);

    // Helper: sanitize position
    out += R"(
const vec2 EPSILON_Z = vec2(0.000001f, -1.00001f);

vec4 SanitizeVertex(vec4 vtx_pos) {
    float ndc_z = vtx_pos.z / vtx_pos.w;
    if (ndc_z > 0.0 && ndc_z < EPSILON_Z.x)
        vtx_pos.z = 0.0;
    if (ndc_z < -1.0 && ndc_z > EPSILON_Z.y)
        vtx_pos.z = -vtx_pos.w;
    return vtx_pos;
}
)";
...
/*std::string GenerateVertexShader(const ShaderSetup& setup, const PicaVSConfig& config, bool separable_shader) {
    std::string out;

    // Always emit #version as the very first line
    out += "#version 310 es\n";

    // Emit supported extensions
    out += R"(
...
#extension GL_ARB_explicit_uniform_location : enable
    ...
    const auto program_source = DecompileProgram(setup.program_code, setup.swizzle_data,
    ...
    for (size_t i = 0; i < used_regs.size(); ++i) {
        if (used_regs[i]) {
            ...
            std::string_view prefix = MakeLoadPrefix(flags);
    ...
    const auto semantic = [&state = config.state](VSOutputAttributes::Semantic slot) -> std::string {
        const u32 slot_id = static_cast<u32>(slot);
        const u32 attrib = state.gs_state.semantic_maps[slot_id].attribute_index;
        const u32 comp = state.gs_state.semantic_maps[slot_id].component_index;
    ...
    if (config.state.use_clip_planes) {
        ...
        out += "    if (enable_clip1) gl_ClipDistance[1] = dot(clip_coef, vtx_pos);\n";
        out += "    else gl_ClipDistance[1] = 0.0;\n";
    }
    out += "    normquat = GetVertexQuaternion();\n";
    out += "    primary_color = vec4(" + semantic(VSOutputAttributes::COLOR_R) + ", " +
